...
public record TransactionResponse(
                ...
                @Schema(description = "URL de callback consommateur pour notifications HTTP") String callbackUrl,
                @Schema(description = "Métadonnées opaques fournies à la création") Map<String, String> metadata,
        ...
        public static TransactionResponse from(Transaction tx, String stripeCheckoutUrl) {
                return new TransactionResponse(tx.id(), tx.walletId(), tx.userId(), tx.organizationId(),
                                tx.amount(), tx.type().name(), tx.status().name(), tx.reference(), tx.fees(),
                                tx.method().name(), tx.stripeSessionId(), stripeCheckoutUrl, tx.callbackUrl(),
                                tx.metadata(),
